<template>
  <div class="space-y-8">
    <div class="card-hard flex flex-col items-start gap-4 bg-yolk p-6 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 id="dashboard-greeting" class="font-display text-3xl font-black">Hey {{ auth.user?.name }} 👋</h1>
        <p class="mt-1 font-semibold">
          {{ auth.isElevated ? `Here's what's on fire across the team.` : `Here's what's on fire today.` }}
        </p>
        <span v-if="auth.isElevated" id="dashboard-role" class="badge mt-2 bg-white">{{ auth.user?.role }}</span>
      </div>
      <div class="flex flex-wrap gap-2">
        <RouterLink v-if="auth.isElevated" id="dashboard-insights" to="/insights" class="btn-hard !bg-sky !text-white">
          📊 QA insights
        </RouterLink>
        <RouterLink v-if="auth.isElevated" id="dashboard-team" to="/users" class="btn-hard !bg-white">🧑‍🤝‍🧑 Team</RouterLink>
        <RouterLink id="dashboard-report-bug" to="/bugs/new" class="btn-hard !bg-punch !text-white">+ Report a bug</RouterLink>
      </div>
    </div>

    <section>
      <h2 id="triage-board-heading" class="mb-3 font-display text-xl font-bold">
      ...
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-5">
        ...
        >
          ...
          <span :id="`triage-count-${lvl.key}`" class="font-display text-2xl font-black">{{ stats?.byPriorityLevel?.[lvl.key] || 0 }}</span>
    ...
    <section class="grid gap-6 sm:grid-cols-2">
      <div class="card-hard p-5">
        ...
        <ul id="dashboard-status-list" class="space-y-2">
          <li v-for="(count, status) in stats?.byStatus" :key="status" :id="`dashboard-status-${status.replace(/\s+/g, '-').toLowerCase()}`" class="flex items-center justify-between">
      ...
      <div class="card-hard p-5">
        ...
        <ul id="dashboard-type-list" class="space-y-2">
    ...
    <section>
      <div class="mb-3 flex items-center justify-between">
        ...
        <RouterLink id="dashboard-see-all" to="/bugs" class="font-bold underline">See all →</RouterLink>
      </div>
      <div v-if="bugsStore.bugs.length" id="dashboard-recent-bugs" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <BugCard v-for="bug in bugsStore.bugs.slice(0, 6)" :key="bug._id" :bug="bug" />
      </div>
      <p v-else id="dashboard-empty" class="font-semibold text-ink/60">No bugs yet. Nice... or suspicious. 👀</p>
    </section>
  </div>
</template>
